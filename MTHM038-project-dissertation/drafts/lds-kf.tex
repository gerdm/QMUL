...
Suppose $\{{\bf z}_n\}_n$ is a signal coming from an unknown source and $\{{\bf x_n}\}_n$ are observed values derived from ${\bf z}_n$. If the system is linear and deterministic we write the equations that govern such system in the form
...
We can also represent our model as a graphical model as specified in Figure \ref{fig:lds-gm}.

\begin{figure}
	\centering
	\input{../figures/lds.tikz}
	\caption{Graphical representation of a linear dynamical system with latent variables.}
	\label{fig:lds-gm}
\end{figure}
